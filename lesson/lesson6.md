...
- Рефакторинг: в конструктор com.javawebinar.webapp.model.Resume добавить второй параметр `fullName`
- Сделать рефакторинг всех реализаций `Storage`: заменить метод `com.javawebinar.webapp.model.Resume[] getAll()` на `List<com.javawebinar.webapp.model.Resume> getAllSorted()`
